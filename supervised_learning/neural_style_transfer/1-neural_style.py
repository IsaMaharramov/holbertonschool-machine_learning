...
class NST:
    ...
    def load_model(self):
        ...
        # Iterate over the layers to replace MaxPool with AvgPool and get targets
        for layer in vgg.layers[1:]:
            ...
            # Store the output tensors for the targeted layers
            if layer.name in self.style_layers + [self.content_layer]:
                outputs[layer.name] = x
                
            # Stop building once we've reached the deepest required layer
            if layer.name == self.content_layer:
                break
                
        # Arrange outputs according to style_layers + [content_layer] order
        output_tensors = [outputs[name] for name in 
                          self.style_layers + [self.content_layer]]
        
        # Initialize our custom sub-model and freeze it
