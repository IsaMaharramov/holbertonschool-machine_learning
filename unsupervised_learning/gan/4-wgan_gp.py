...
class WGAN_GP(keras.Model):
    ...
    def __init__(self, generator, discriminator, latent_generator,
        ...
        """ Initialize the WGAN Gradient Penalty model """
        super().__init__()
        ...
        self.discriminator.loss = lambda x, y: (tf.reduce_mean(x) -
                                                tf.reduce_mean(y))
    ...
    def train_step(self, useless_argument):
        ...
        for _ in range(self.disc_iter):
            with tf.GradientTape() as disc_tape:
                ...
                inter_sample = self.get_interpolated_sample(
                ...
                gp = self.gradient_penalty(inter_sample)
                new_discr_loss = discr_loss + self.lambda_gp * gp

            grad_d = disc_tape.gradient(
                new_discr_loss,
                self.discriminator.trainable_variables)
            self.discriminator.optimizer.apply_gradients(
                zip(grad_d, self.discriminator.trainable_variables))
        ...
        grad_g = gen_tape.gradient(
            gen_loss,
            self.generator.trainable_variables)
        self.generator.optimizer.apply_gradients(
            zip(grad_g, self.generator.trainable_variables))

        return {"discr_loss": discr_loss, "gen_loss": gen_loss, "gp": gp}

    def replace_weights(self, gen_h5, disc_h5):
        """ Replaces the initialized weights with pre-trained H5 files """
